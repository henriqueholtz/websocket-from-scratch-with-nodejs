...
function prepareMessage(message) {
  ...
  if (messageSize <= SEVEN_BITS_INTEGER_MARKER) {
    ...
    let offsetFoursBytes = 4;
    const target = Buffer.allocUnsafe(offsetFoursBytes);
    target[0] = firstByte;
    ...
    // alloc 4 bytes
    // [0] -> 128 + 1 -> 10000001 = 0x01 fin + opcode
    // [1] -> 126 + 0 -> payload length marker + mask indicator
    // [2] -> 0 -> content length
    // [3] -> 140 -> content length
...
function onSocketReadable(socket) {
  ...
  if (lengthIndicatorInBits <= SEVEN_BITS_INTEGER_MARKER) {
    ...
    messageLength = socket.read(2).readUint16BE(0); //2 bytes
  } else {
